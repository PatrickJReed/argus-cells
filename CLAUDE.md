...
Three-headed (Cerberus-inspired) ResNet34 with a shared image encoder and three task heads: cell-type classification, **organelle soft-segmentation** of 5 Cell Painting compartments from brightfield (each fluorescence channel acts as a soft probability mask: high fluorescence → high probability that pixel belongs to that organelle), and disease-state classification on 22q11.2 deletion vs control iPSC lines. v0 trains this alongside an all-channel single-task disease-classifier baseline (same ResNet34 encoder, 6-channel input, single line-condition head) so the Cerberus disease number is reported against a meaningful upper bound: "what fraction of the all-channel disease signal is recoverable from brightfield alone, at 1/6 the assay cost?". The organelle segmentation head also serves as a model-interpretability lever for the disease classifier: predicted organelle masks can be compared between control and 22q11.2-deletion cells to recapitulate the Tegtmeyer 2025 finding (mitochondrial structure changes in deletion astrocytes) via a brightfield-only prediction model.
